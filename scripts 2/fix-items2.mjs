...
async function main() {
    ...
    // Her merge işlemi updates nesnesini günceller, items'i de in-memory günceller
    const mergeInto = (keeperName, dupName) => {
        ...
        // Güncel miktarı al (önceki merge güncellemiş olabilir)
        const currentKeeperQty = updates[`items/${keeperKey}/quantity`] ?? Number(keeperItem.quantity) ?? 0;
        const newQty = currentKeeperQty + (Number(dupItem.quantity) || 0);

        const keeperMovs = Object.values(movements).filter(m => String(m.itemId) === String(keeperItem.id) && Number(m.birimFiyat) > 0);
        const dupMovs    = Object.values(movements).filter(m => String(m.itemId) === String(dupItem.id)    && Number(m.birimFiyat) > 0);
        ...
        console.log(`  "${dupName}" → "${keeperName}" | qty ${currentKeeperQty}+${dupItem.quantity}=${newQty}, ${moved} hareket`);
    };

    console.log('ÇEKİÇ birleştirme:');
    mergeInto('ÇEKİÇ', 'ÇEKİÇ - AHŞAP SAPLI 1000GR');

    console.log('\nEMNİYET ŞERİDİ birleştirme:');
    mergeInto('EMNİYET İKAZ ŞERİDİ - 360 MT', 'EMNİYET İKAZ ŞERİDİ 360 MT');
    mergeInto('EMNİYET İKAZ ŞERİDİ - 360 MT', 'EMNİYET ŞERİDİ (KIRMIZI-BEYAZ)');
